...
import {ERC20MintablePausableBurnable} from "../../../src/iter/ccip/ITERX.sol";
...
contract DeployStandard is Deployer {
    ERC20MintablePausableBurnable public iter;

    function run() external {
        _setDeployer();
        iter = new ERC20MintablePausableBurnable();
        vm.stopBroadcast();
    }
}
...
contract GrantMinterRole is Deployer {
    ...
    function run() external {
        _setDeployer();
        iter = ERC20MintablePausableBurnable(iter_address);
