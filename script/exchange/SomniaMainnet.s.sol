...
import {WSOM9} from "../../src/mock/WSOM9.sol";
...
contract initialize is Deployer {
    address constant engine_address = 0x00cB733CBF6fb7079eeeC9EA9b50863756dDbfBE;
    address constant orderbookFactory_address = 0xd7ABA1cbAd246249be6a0de9a449FB5EDEFf1E47;
    address constant deployer_address = 0xF8FB4672170607C95663f4Cc674dDb1386b7CfE0;
    address constant weth_address = 0x76190B0EfA07F183c8B06576E9E9ab7aFb5b94e9;

    function run() external {
        _setDeployer();
        MatchingEngine matchingEngine = MatchingEngine(payable(engine_address));
        OrderbookFactory orderbookFactory = OrderbookFactory(orderbookFactory_address);
        orderbookFactory.initialize(address(matchingEngine));
        matchingEngine.initialize(address(orderbookFactory), address(deployer_address), address(weth_address));
    }
}
