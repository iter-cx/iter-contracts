...
/// @title ITER Membership registration and subscription
contract PointFarm is AccessControl, Initializable {
    ...
    function setITER(address iter) external {
        if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
            revert InvalidRole(DEFAULT_ADMIN_ROLE, msg.sender);
        }
        _membership._setITER(iter);
    }
    ...
    function getSubITER(uint256 uid_) external view returns (uint64) {
        return _membership._getSubITER(uid_);
    }
