...
library MembershipLib {
    struct Member {
        ...
        /// @dev mapping of subscribed ITER of member id
        mapping(uint256 => uint64) subITER;
    ...
    function _setMeta(Member storage self, uint256 uid_, uint8 metaId_) internal {
        IPass(self.pass).setMeta(uid_, metaId_);
    }

    function _setITER(Member storage self, address iter) internal {
        self.iter = iter;
    }
    ...
    function _register(Member storage self, uint8 metaId_, address feeToken_, bool isDev)
        internal
        returns (uint256 uid)
    {
        if (!isDev) {
    ...
    function _subscribe(Member storage self, uint256 uid_, address feeToken_, uint64 blocks_)
        internal
        returns (uint256 at, uint256 until, address with)
    {
        ...
        // if feeToken is ITER, add it to the subITER;
        if (feeToken_ == self.iter) {
            uint8 decimals = TransferHelper.decimals(self.iter);
            self.subITER[uid_] += uint64((fees.subFee * blocks_) / 10 ** decimals);
        }
    ...
    function _unsubscribe(Member storage self, uint256 uid_) internal {
        ...
        // subtract subITER if ITER was used to subscribe
        if (sub.with == self.iter) {
            self.subITER[uid_] -= uint64((fees.subFee * (sub.until - bh - sub.bonus)) / 1e18);
        }
    }

    /// @dev offerBonus: Offer trial blocks to the subscription by promoters
    function _offerTrial(Member storage self, uint256 uid_, address holder_, uint256 blocks_) internal {
        // check if the member has the ABT with input id
    ...
    function getFees(Member storage self, uint8 metaId_, address feeToken)
        internal
        view
        returns (MembershipLib.Fees memory fee)
    {
        return self.fees[metaId_][feeToken];
    }

    function _balanceOf(Member storage self, address owner_, uint256 uid_) internal view returns (uint256) {
        return IPass(self.pass).balanceOf(owner_, uid_);
    }

    function _isSubscribed(Member storage self, address account) internal view returns (bool) {
    ...
    function _getSubITER(Member storage self, uint256 uid_) internal view returns (uint64) {
        return self.subITER[uid_];
    }

    function _getSubStatus(Member storage self, uint256 uid_) internal view returns (SubStatus memory status) {
    ...
    function _getLvl(Member storage self, uint256 uid_) internal view returns (uint8) {
    ...
    function _sendFunds(Member storage self, address token, address to, uint256 amount) internal returns (bool) {
        if (token == self.weth) {
    ...
    function _getPointX(Member storage self, uint256 uid) internal view returns (uint32 feeNum) {
        if (uid == 0) {
            return 10000;
        }
        uint8 level = _getLvl(self, uid);
        // get subscribed ITER tokens
        uint64 subITER = _getSubITER(self, uid);
    ...
    function _getFeeRate(Member storage self, address account, bool isMaker) internal view returns (uint32 feeNum) {
        uint256 uid = self.defaultUIDs[account];
        uint8 level = _getLvl(self, uid);
        // get subscribed ITER tokens
        uint64 subITER = _getSubITER(self, uid);
