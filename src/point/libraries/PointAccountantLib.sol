 ...
 * referrals at all.
 *
 * Known defects, recorded because the file stays in the tree:
 *  - _setBaseMultiplier declares `returns (uint32)` with no return statement.
 *  - _report computes snd/onr, mints them, then returns an unassigned `points`
 *    — always 0.
 *  - The stablecoin-value branch calls IPoint.fine() (a PENALTY) where the
 *    surrounding code appears to intend an award.
 *  - `baseMultiplier * _getMultiplier(...)` is uint32 x uint32 into a uint32;
 *    multipliers above roughly 429x overflow.
 */
