...
interface IMatchingEngine {
    ...
    //
    // The ORDERING is the load-bearing part, not the names: these values cross an ABI
    // boundary as a uint8, so a member here that the engine does not have in the same
    // position sends the wrong mode with no type error anywhere. SizePriority was removed
    // from the engine on 2026-08-06 and is removed here in the same change.
    enum MatchingMode {
        PriceTimePriority
    }
    ...
    // user functions
    function marketBuy(address base, address quote, uint256 quoteAmount, bool isMaker, uint32 n, address recipient)
        external
        returns (uint256 makePrice, uint256 placed, uint32 id);

    function marketSell(address base, address quote, uint256 baseAmount, bool isMaker, uint32 n, address recipient)
        external
        returns (uint256 makePrice, uint256 placed, uint32 id);

    function marketBuyETH(address base, bool isMaker, uint32 n, address recipient)
        external
        payable
        returns (uint256 makePrice, uint256 placed, uint32 id);

    function marketSellETH(address quote, bool isMaker, uint32 n, address recipient)
        external
        payable
        returns (uint256 makePrice, uint256 placed, uint32 id);
