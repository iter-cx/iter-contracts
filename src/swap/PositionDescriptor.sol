...
/// @notice Fully on-chain `tokenURI` for ITER swap liquidity positions.
...
contract PositionDescriptor {
        ...
        returns (string memory)
    {
        string memory head = string(
            abi.encodePacked(
                '{"name":"ITER LP ', v.pair, " #", v.tokenId, '","description":"', _description(v)
            )
        );
        return string(
            abi.encodePacked(
    ...
    function _description(PositionSVG.Params memory v) internal pure returns (string memory) {
        string memory head = string(
            abi.encodePacked(
                "Liquidity position on the ITER ",
                v.pair,
