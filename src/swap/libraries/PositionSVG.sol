...
/// @title PositionSVG
/// @notice Pure on-chain renderer for off-grid swap liquidity positions.
...
library PositionSVG {
    ...
    /// @dev The survey grid the protocol is named for. It is drawn only across the
    ///      top of the card and then faded out — the artwork goes off-grid.
    ...
    function _header(Params memory p) private pure returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                _text(24, 42, eyebrow, "OFF-GRID LIQUIDITY"),
