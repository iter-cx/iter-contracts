...
contract ConversionTest is BaseSetup {
    ...
    function testInvalidConversion() public {
        super.setUp();
        matchingEngine.addPair(address(token1), address(token2), 100e8, 0, address(token1), ExchangeOrderbook.MatchingMode.PriceTimePriority);
    ...
    function testConvertBuySellOnDifferentDecimalWhereBaseBQuote() public {
        super.setUp();
        matchingEngine.addPair(address(token1), address(btc), 1000e8, 0, address(token1), ExchangeOrderbook.MatchingMode.PriceTimePriority);
        ...
        // Trader2 crossed the resting bid, so it pays the taker fee: 1e12 less 0.1%.
        ...
        // Trader1 rested first, so it is the maker and receives its base GROSS --
        // makers pay no fee (MatchingEngine.defaultMakerFee). Was 999e16.
        assert(diffTrader1T1Balance == 1e19);
    ...
    function testConvertSellBuyOnDifferentDecimalWhereBaseBQuote() public {
        ...
        // Trader2 rested first, so it is the maker and receives its quote GROSS --
        // makers pay no fee (MatchingEngine.defaultMakerFee). Was 999e9.
        assert(diffTrader2BTCBalance == 1e12);
        // Trader2's token1 balance should be decreased by 10e18
        assert(diffTrader2T1Balance == 1e19);
        // Trader1's btc balance should be decreased by 10000e8
        assert(diffTrader1BTCBalance == 1e12);
        // Trader1 crossed the resting ask, so it pays the taker fee: 1e19 less 0.1%.
    ...
    function testConvertBuySellOnDifferentDecimalWhereNotBaseBQuote() public {
        super.setUp();
        matchingEngine.addPair(address(btc), address(token2), 1000e8, 0, address(btc), ExchangeOrderbook.MatchingMode.PriceTimePriority);
        ...
        // Trader1 rested first, so it is the maker and receives its base GROSS --
        // makers pay no fee (MatchingEngine.defaultMakerFee). Was 999e6.
        assert(diffTrader1BTCBalance == 1e9);
    ...
    function testConvertSellBuyOnDifferentDecimalWhereNotBaseBQuote() public {
        ...
        // Trader2 rested first, so it is the maker and receives its quote GROSS --
        // makers pay no fee (MatchingEngine.defaultMakerFee). Was 999e19.
        assert(diffTrader2T2Balance == 1e22);
