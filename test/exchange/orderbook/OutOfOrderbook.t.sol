...
contract OutOfOrderbookTest is BaseSetup {
    ...
    function testTradeWithPriceZeroDoesNotWork() public {
        super.setUp();
        matchingEngine.addPair(address(token1), address(token2), 100, 0, address(token1), ExchangeOrderbook.MatchingMode.PriceTimePriority);
        vm.prank(trader1);
        vm.expectRevert();
        matchingEngine.limitBuy(address(token1), address(token2), 0, 990, true, 2, trader1);
        vm.prank(trader2);
        vm.expectRevert();
        matchingEngine.limitSell(address(token1), address(token2), 0, 990, true, 2, trader2);
    }
