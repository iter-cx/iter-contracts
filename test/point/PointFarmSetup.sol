...
contract PointFarmSetup is Test {
    ...
    function setUp() public {
        ...
        matchingEngine.addPair(address(base), address(usdc), 341320000000, 0, address(base), ExchangeOrderbook.MatchingMode.PriceTimePriority);
        // make a price in matching engine where 1 feeToken = 1000 stablecoin with buy and sell order
        matchingEngine.addPair(address(feeToken), address(stablecoin), 10000e8, 0, address(feeToken), ExchangeOrderbook.MatchingMode.PriceTimePriority);
