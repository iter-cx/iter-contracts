...
contract SubscriptionTest is PointFarmSetup {
    ...
    // subscribing with iter shows subscribed ITER amount
    function testSubITERIsChangedOnITERSubscription() public {
        subcSetUp();
        vm.startPrank(trader1);
        pointFarm.setITER(address(feeToken));
        pointFarm.register(9, address(feeToken));
        pointFarm.subscribe(1, address(feeToken), 10000);
        uint256 subITER = pointFarm.getSubITER(1);
        assert(subITER == 10000);
        vm.stopPrank();
    }
